// MODIFIED (Mel Krusniak): kernel/sys.c
//   Inserted memuse function on lines 2669 to 2684.
...
SYSCALL_DEFINE2(memuse, pid_t, pid, int, mem_type) {
	...
	if (!p) { // check PID
		pr_alert("memuse: custom syscall could not find process with specified pid\n");
		return -1;
